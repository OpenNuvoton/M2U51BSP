...
uint32_t GetUartBaudrate(UART_T *uart)
{
    ...
    uint32_t u32ClkTbl[] = {0ul, __MIRC, __HIRC, __LIRC, __LXT};
    ...
    case UART0_BASE:
        ...
        u8UartClkDivNum = (CLK->CLKDIV & CLK_CLKDIV_UART0DIV_Msk) >> CLK_CLKDIV_UART0DIV_Pos;
        break;
    case UART1_BASE:
        u8UartClkSrcSel = (CLK->CLKSEL2 & CLK_CLKSEL2_UART1SEL_Msk) >> CLK_CLKSEL2_UART1SEL_Pos;
        u8UartClkDivNum = (CLK->CLKDIV & CLK_CLKDIV_UART1DIV_Msk) >> CLK_CLKDIV_UART1DIV_Pos;
        break;
    case UART2_BASE:
        u8UartClkSrcSel = (CLK->CLKSEL2 & CLK_CLKSEL2_UART2SEL_Msk) >> CLK_CLKSEL2_UART2SEL_Pos;
        u8UartClkDivNum = (CLK->CLKDIV & CLK_CLKDIV_UART2DIV_Msk) >> CLK_CLKDIV_UART2DIV_Pos;
        break;
    default:
        return 0;
    }

    /* Get PCLK or MIRC clock frequency if UART clock source selection is PCLK */
    if (u8UartClkSrcSel == 0ul)
    {
    ...
    else if (u8UartClkSrcSel == 1ul)
    {
        u32ClkTbl[u8UartClkSrcSel] = CLK_GetMIRCFreq();
    }
